...
# 检查 SceneBuilder 问题
SCENE_BUILDER_ISSUES=$(find WorkbenchApp -name "*.swift" -exec grep -l "if.*#available.*SceneBuilder\|SceneBuilder.*if.*#available" {} \; 2>/dev/null | wc -l)
if [ $SCENE_BUILDER_ISSUES -gt 0 ]; then
    echo -e "${YELLOW}⚠️  发现 $SCENE_BUILDER_ISSUES 个文件有 SceneBuilder 问题${NC}"
else
    echo -e "${GREEN}✅ 没有发现 SceneBuilder 问题${NC}"
fi
...
echo -e "${BLUE}SceneBuilder 问题: $SCENE_BUILDER_ISSUES${NC}"

TOTAL_ISSUES=$((IOS14_ISSUES + IOS16_ISSUES + APP_ISSUES + SCENE_BUILDER_ISSUES))
